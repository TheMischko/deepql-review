...
from agents.deepq_l.IDeepQLAgent import IDeepQLAgent
from agents.deepq_l.neural_net import NeuralNet, get_optimizer


class DDQ_l(IDeepQLAgent):
    def __init__(self, refm, disc_rate, learning_rate, gamma, batch_size, epsilon, epsilon_decay_length, neural_size_l1,
        ...
        IDeepQLAgent.__init__(self, refm, disc_rate, learning_rate, gamma, batch_size, epsilon, epsilon_decay_length,
    ...
    def __str__(self):
        if self.eligibility_strategy is not None:
            return "DDQ_l(%.4f,%.2f,%d,%.3f,%d,%d,%d,%d,%d,%.3f,%d,%.3f,%d)" % (
                self.learning_rate,
                self.gamma,
                self.batch_size,
        ...
        return "DDQ_l(%.4f,%.2f,%d,%.3f,%d,%d,%d,%d,%d,%.3f,%d)" % (
            self.learning_rate,
            self.gamma,
            self.batch_size,
            self.starting_epsilon,
